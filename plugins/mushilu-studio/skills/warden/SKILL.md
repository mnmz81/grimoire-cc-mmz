...
**Requires (target repo):** the enforcing hooks (`freeze.mjs`, `careful.mjs`, …) and their
Claude Code settings wiring live in the **project**, not in this plugin — this plugin ships the
*operator*, not the guards. If the repo has no such hooks, Warden's marker is inert: say so
rather than implying edits are locked. The hooks live in the project's `scripts/` (`team-hooks`
folder) and are wired into the Claude Code settings file. They always run; Warden only changes
the markers two of them consult:
